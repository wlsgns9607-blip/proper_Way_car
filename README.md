...
### 1. 프로젝트 개요 (Overview)
기존 모바일 앱 중심의 세차 예약 서비스를 스마트폰 화면(In-App)에만 가두지 않고, 차량 인포테인먼트 시스템(**Android Auto / Apple CarPlay**)과 USB 및 무선으로 연동되도록 반응형 Layout 스펙을 확장했습니다. 운전석에 앉아 차량 내비게이션 화면만으로 실시간 도장면 케어 알림, 세차장 예약, 자동 체크인까지 원스톱으로 해결하는 **'드라이버 중심의 실전형 카케어 UX 플랫폼'**입니다.
...
* **차량 디스플레이 최적화 1-Touch 예약/결제 (In-Car Reservation)**:
  * *드라이빙 안전 가이드 적용*: 주행 중 조작을 최소화하기 위해 큼직하고 시인성 높은 버튼 배치(Big Button UI)를 마크업했습니다.
