...
제거 방법: 절대 힘으로 벅벅 문지르지 마세요. 알코올(소독용 에탄올)이나 타르/수액 제거제를 막타월에 듬뿍 적신 뒤 수액 위에 가볍게 얹어두세요. 성분이 녹아 말랑해졌을 때 부드럽게 걷어내듯 닦아내야 흠집 없이 안전하게 지워집니다. 다시 한번 강조하지만, 시간이 지나면 도장면 자체가 파고들어 영구 변색되니 무조건 빨리 지우셔야 합니다.`
  ...
  // 질문: 벌레제거제로 쓰면 좋은 세차용품
  else if (userText.includes("벌레제거제") || (userText.includes("벌레") && userText.includes("용품") && userText.includes("추천")) || (userText.includes("벌레") && userText.includes("제거제"))) {
    return {
      text: `버그클리너가 있습니다. 대중적으로 사용합니다. 
      
만약 안 지워지는 게 있다면 PB나 알칼리성(또는 pH 2 계열의 산성) 제품을 쓰시면 더 잘 지워지긴 하지만, 도장면 코팅에 좋지 않기 때문에 웬만하면 사용하지 않는 걸 추천합니다.`
    };
  }
  // 질문 6번 모범 답변
...
제거 방법: 단단히 굳은 벌레들은 고압수만으론 안 떨어집니다. 세차 시작 전 버그 크리너를 범퍼와 사이드미러에 미리 뿌려 단백질을 먼저 녹이세요. 그 후 고압수로 큰 덩어리를 1차로 날려버린 뒤, 반드시 스노우폼을 뿌려 부드러운 미트질로 남은 미세 잔여물과 파고든 기름때까지 완벽하게 닦아내야 도장면에 대미지 없이 깔끔하게 끝납니다.`
  ...
  // 질문: 가성비 세차용품 추천
  else if (userText.includes("가성비세차용품") || (userText.includes("가성비") && userText.includes("용품")) || (userText.includes("세차용품") && userText.includes("추천"))) {
    return {
      text: `1. **카샴푸 (미트질용)**
추천: 소낙스(SONAX) 세차샴푸 (글로스 샴푸)

이유: 가성비 카샴푸의 영원한 근본입니다. 1리터 대용량에 만 원 초반대인데, 거품 풍성하고 윤활력이 좋아서 도장면 데미지 없이 때를 아주 부드럽게 밀어버립니다. 희석비도 좋아서 한 통 사면 1년은 씁니다.

2. **휠 & 타이어 (하체 관리)**
추천: 소낙스 익스트림 휠클리너 또는 더클래스 휠앤타이어

이유: 분진 많은 수입차나 찌든 때 휠에 뿌리면 보라색 피눈물 흘리면서 갈변과 철분을 한 방에 녹여버립니다. 휠이랑 타이어 따로 살 필요 없이 올인원으로 끝내기에 이만한 가성비가 없습니다.

3. **물왁스 / 물광 코팅 (최종 마무리)**
추천: 더클래스 불렛 (Bullet) 타이거 또는 루미너스 LM 그래핀 에디션

이유: 요즘 국산 세차 용품들이 진짜 약 빨고 잘 만듭니다. 슬릭감(만졌을 때 미끄러지는 느낌)이랑 광택감이 미쳤습니다. 세차 다 하고 물기 닦은 다음 슥슥 바르고 타월로 밀어주면, 거울처럼 반사되는 광이 올라옵니다. 작업성도 좋아서 잔사(얼룩)가 잘 안 남습니다.

4. **유리세정제 (잔사 방지)**
추천: 인비저블 글래스 ( Invisible Glass - 보라색 외관)

이유: 틴팅지 상하게 하는 암모니아가 없고, 닦고 나서 유리에 허옇게 줄 생기는 잔사가 아예 안 남는 걸로 유명합니다. 유리 닦을 때 스트레스 안 받으려면 무조건 이거 사시면 됩니다.`
    };
  }
  // 질문 9번 모범 답변
...
드라잉 및 물왁스 마감: 큰 세차 타월을 펼쳐 외관 물기를 슥 쓸어내리며 잡고, 외관용 타월로 유리부터 맑게 닦아냅니다. 마지막으로 성능 좋은 물왁스를 사용하여 도장면 전체를 꼼꼼하게 버핑해 주면 번쩍이는 프리미엄 불광이 완성됩니다.`
  ...
  return {
    text: "현재 세차 AI 전문 기능은 **포트폴리오 시연 모드**로 작동 중입니다.\n\n예시 질문:\n1. '동선이 꼬이지 않고 빨리할 수 있는 세차 방법을 알려줘'\n2. '에어건으로 실내 먼지 털 때 가장 빠르고 디테일하게 하는 방법은?'\n3. '30분 초고속 세차 방법'\n4. '나무 수액 제거'\n5. '새똥 제거'\n6. '벌레제거제로 쓰기 좋은 세차용품은 뭐야?'\n7. '벌레 사체 지우기'\n8. '가죽 시트 관리법'\n9. '고압수 각도'\n10. '가성비 세차 방법은 뭐야?'\n11. '가성비 세차용품 추천해줘'\n12. '엔진룸 청소해도 괜찮아?'\n13. '타이어 갈변 해결법'\n14. '매트 세척건조기 사용 팁'\n15. '잔기스 컴파운드로 지워져?'\n16. '자동세차 자주 돌리면 망가져?'\n17. '겨울철 세차 팁'\n18. '개인 용품 사용 및 구매 팁'\n19. '타르 지우는 법'\n20. '지하주차장 시멘트물 떨어졌을 때 대처법'\n21. '하이그로시 내장재 기스 없이 닦는 법'\n22. '가장 효과적으로 고압수 뿌리는 방법은 뭐야?'\n23. '플라스틱 트림이 하얗게 바랬는데 어떡해?'\n24. '여름철 세차할 때 가장 주의해야 할 점은 뭐야?'\n25. '세차 수건은 어떻게 세탁해야 해?'\n26. '유리막 코팅 차량 관리법'\n27. '차 담배 냄새 제거법'\n28. '스노우폼 뿌리고 대기 시간'\n29. '하부 세차는 꼭 해야 해?'\n30. '물왁스 바를 때 꿀팁'\n31. '초보자가 세차할 때 가장 먼저 갖춰야 할 기본 습관은?'\n32. '세차장 공용 거품 솔 써도 돼?'\n33. '프리미엄 세차 순서와 방법'\n\n키워드를 포함해 질문해 보시면 전문가 수준의 답변을 확인하실 수 있습니다!"
  };
};
        ...
        <motion.div variants={item} className="bg-white rounded-[2.5rem] md:rounded-[3.5rem] p-6 md:p-12 shadow-md border border-white/50 mb-10 overflow-hidden">
          ...
          <div className="flex flex-col gap-8">
            <div className="flex justify-between items-center bg-slate-50/50 p-2 rounded-full pr-4">
              <span className="text-slate-500 font-black text-base ml-2">오늘의 미세먼지</span>
              {loadingWeather ? (
                <span className="text-slate-300 text-sm animate-pulse">정보를 불러오는 중...</span>
              ) : (
                <span className={`px-5 py-2 rounded-full text-sm font-black shadow-sm ${
