...
function AuthScreen({ onLogin, onBack }: { onLogin: (u: UserProfile) => void, onBack?: () => void }) {
  ...
  const handleNaverLogin = async () => {
    try {
      ...
      if (!clientId) {
        alert('³×ÀÌ¹ö ·Î±×ÀÎÀÌ È°¼ºÈ­µÇÁö ¾Ê¾Ò½À´Ï´Ù.\\n¾Û ¼³Á¤¿¡¼­ VITE_NAVER_CLIENT_ID¸¦ ¼³Á¤ÇØÁÖ¼¼¿ä.');
        return;
      }
      ...
      const url = \https://nid.naver.com/oauth2.0/authorize?response_type=code&client_id=\&redirect_uri=\&state=\\;
      ...
      window.open(url, 'naver_oauth_popup', \width=\,height=\,left=\,top=\\);
    } catch (err: any) {
      console.error('[Auth] Naver URL error:', err);
      alert('³×ÀÌ¹ö ·Î±×ÀÎ ÁØºñ Áß ¿À·ù°¡ ¹ß»ıÇß½À´Ï´Ù.');
    }
  };
  ...
  const handleKakaoLogin = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) {
        throw new Error(data.error || "ì¹´ì¹´??ë¡œê·¸??ì´ˆê¸°?”ì— ?¤íŒ¨?ˆìŠµ?ˆë‹¤.");
      }
      ...
      if (err.message.includes("?¤ì •?˜ì? ?Šì•˜?µë‹ˆ??)) {
        alert("ì¹´ì¹´??ë¡œê·¸?¸ì´ ?œì„±?”ë˜ì§€ ?Šì•˜?µë‹ˆ??\n\n???¤ì •(Settings) ë©”ë‰´?ì„œ VITE_KAKAO_CLIENT_ID?€ KAKAO_CLIENT_SECRET???¤ì •?´ì£¼?¸ìš”.");
      } else {
        alert("ì¹´ì¹´??ë¡œê·¸???¤íŒ¨: " + err.message);
      }
    } finally {
      setLoading(false);
    }
